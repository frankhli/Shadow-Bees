...
export default function VenueEarningsPage() {
  ...
  const completedOrders = venueOrders.filter(o => o.status === 'completed')
  ...
  const withdrawn = transactions
    .filter(t => t.type === 'withdrawal' && t.userId === 'venue1' && t.status === 'completed')
  ...
  const pendingWithdrawals = transactions.filter(
    t => t.type === 'withdrawal' && t.userId === 'venue1' && t.status === 'pending'
  )

  // 提现记录
  const withdrawalHistory = transactions.filter(
    t => t.type === 'withdrawal' && t.userId === 'venue1'
  )
